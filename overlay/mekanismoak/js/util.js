// Mekanismoen Lantegia — tresna partekatuak
...
export function parseNum(raw) {
  const s = String(raw).trim().replace(/\s/g, '').replace(/[a-zA-Z²³°/]+$/g, '').replace(',', '.');
...
// Graduatzailea + irteera; `onInput(balioa)` deitzen du
...
const KEY = 'mekanismoak:aurrerapena:v1';
function load() {
  try { return JSON.parse(localStorage.getItem(KEY)) || {}; } catch { return {}; }
}
function save(data) {
  try { localStorage.setItem(KEY, JSON.stringify(data)); } catch { /* nabigatzaile pribatua: memorian bakarrik */ }
}
let cache = load();
export const progress = {
  get(unit) { return cache[unit] || {}; },
  set(unit, patch) {
    cache[unit] = { ...cache[unit], ...patch };
    save(cache);
    document.dispatchEvent(new CustomEvent('aurrerapena'));
  },
  all() { return cache; },
  replace(data) { cache = data || {}; save(cache); document.dispatchEvent(new CustomEvent('aurrerapena')); }
};
